...
const AWESOME_VIM_REPO = "akrawchyk/awesome-vim";
const logger = createLogger({ context: "awesome-vim-crawler-v2" });

function parseAwesomeVimReadme(
  ...
  for (const line of lines) {
    ...
    // Start processing when we reach the Colors section
    if (line.trim() === "## Colors") {
      inTargetSection = true;
      currentTags = ["Colors"]; // Set initial tag when starting
      logger.info(`Found Colors section at line ${processedLines}`);
      continue;
    }

    if (!inTargetSection) continue;

    // Handle XML section and process its repositories, then stop
    if (line.trim() === "### XML") {
      logger.info(`Reached XML section at line ${processedLines}`);
      currentTags = [currentTags[0], "XML"]; // Set XML as subcategory
      continue; // Continue processing to get XML repositories
    }

    // Extract repositories first, then check for stop condition
    ...
    // Stop when we reach the next main section after XML
    if (inTargetSection && line.trim() === "## Framework Specific") {
      logger.info(`Reached Framework Specific section - stopping after XML`);
      break;
    }
    ...
    if (headingMatch) {
      ...
      if (title.includes("requires") || title.includes("back to top")) {
        continue;
      }

      // Adjust tags array based on heading level
      if (level === 2) {
        // Main category (## Colors, ## Tools, etc.)
        currentTags = [title];
        logger.debug(`Processing category: ${title}`);
      } else if (level === 3) {
        // Subcategory (### Editing, ### File Management, etc.)
    ...
    // Repository processing is now handled above
  }
...
export async function crawl(): Promise<Map<string, GithubRepository>> {
  logger.info("Starting awesome-vim crawler");

  // Fetch README from awesome-vim repository
  const readmeResult = await getRepositoryReadme({
    full_name: AWESOME_VIM_REPO,
    branch: "master",
  } as unknown as any);
  if ("error" in readmeResult) {
    throw new Error(
      `Failed to fetch README from ${AWESOME_VIM_REPO}: ${readmeResult.error}`,
    );
  }

  // Parse repositories from README
  const parsedRepos = parseAwesomeVimReadme(readmeResult.data);
