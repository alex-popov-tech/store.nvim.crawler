    ...
    matcher: (
      ...
      if (tableValue.value?.type !== "StringLiteral") {
        ...
        if (tableValue.value?.type === "TableConstructorExpression") {
          ...
          if (
            !nestedPluginName.toLowerCase().includes(expectedRepo.toLowerCase())
          )
            return null;
      ...
      const pluginNameOrUrl =
        stringLiteral.raw?.slice(1, -1) || stringLiteral.value;

      if (!pluginNameOrUrl.toLowerCase().includes(expectedRepo.toLowerCase()))
        return null;
